"""
Dash Batch Monitor ─ live Arduino version (FIXED)
────────────────────────────────────────────────────────────
Features
...
SERIAL_PORT = "COM10"   # 🔁 change to your actual port
BAUD_RATE = 9600
RECONNECT_DELAY = 3  # seconds
MAX_RETRIES = 5

def open_serial():
    """Safely open serial connection with retries"""
    ...
    while retry_count < MAX_RETRIES:
        with serial_lock:
            try:
                ...
                # Give Arduino time to initialize and clear any buffered data
                time.sleep(2)
                ...
                retry_count += 1
                print(f"[ERROR] Cannot open {SERIAL_PORT} (attempt {retry_count}/{MAX_RETRIES}): {e}")
...
def read_temperature():
    global serial_instance                        # ← first line in function

    with serial_lock:
        if not serial_instance or not serial_instance.is_open:
            return None

        try:
            line = serial_instance.readline().decode("utf-8", errors="ignore").strip()

            # Skip the Arduino banner

            temp = float(line)

            if -200 <= temp <= 1350:               # plausible MAX6675 range
                return temp
            else:
                print(f"[WARNING] Temperature out of range: {temp}°C")
                return None

        except (ValueError, UnicodeDecodeError) as e:
            print(f"[WARNING] Could not parse temperature: {e}")
            return None
...
def serial_worker():
    ...
    global serial_instance, current_batch_id
    
    print("[INFO] Starting serial worker thread...")
        ...
        try:
            ...
            if temp_c is None:
                ...
                if consecutive_failures >= max_consecutive_failures:
                    print(f"[WARNING] {consecutive_failures} consecutive read failures, reconnecting...")
                    with serial_lock:
                        if serial_instance:
                            serial_instance.close()
                        serial_instance = None
            ...
            # Get current batch ID safely
            with serial_lock:
                bid = current_batch_id
            
            if not bid:
                # No active batch, but we can still print the temperature
                print(f"[DEBUG] Temperature: {temp_c:.2f}")
                time.sleep(1)
                continue
            ...
            try:
                with get_conn() as con, con.cursor() as cur:
                    ...
                    con.commit()
                    print(f"[DEBUG] Logged: {temp_c:.2f}°C for batch {bid}")
            except Exception as db_error:
                print(f"[ERROR] Database error: {db_error}")
            
        except serial.SerialException as ser_err:
            print(f"[ERROR] Serial communication error: {ser_err}")
            consecutive_failures += 1
            ...
            print(f"[ERROR] Unexpected error in serial worker: {e}")
            time.sleep(1)
...
def live_graph(_, bid):
    if bid:
        query = """
            ...
            WHERE batch_id = %s AND temperature_c BETWEEN -200 AND 1350
        ...
        query = """
            SELECT timestamp, temperature_c FROM temperature_log
            WHERE temperature_c BETWEEN -200 AND 1350
            ORDER BY timestamp DESC LIMIT 100
        """
        params = None
...
if __name__ == "__main__":
    print("[INFO] Starting Dash Batch Monitor...")
    print(f"[INFO] Serial port configured: {SERIAL_PORT} @ {BAUD_RATE}")
    threading.Thread(target=serial_worker, daemon=True).start()
    app.run(debug=True, use_reloader=False)  # 👈 prevents double thread
